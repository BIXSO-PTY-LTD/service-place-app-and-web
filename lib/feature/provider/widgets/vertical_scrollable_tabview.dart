...
class VerticalScrollableTabViewState extends State<VerticalScrollableTabView> with SingleTickerProviderStateMixin {
  ...
  bool onScrollNotification(ScrollNotification notification) {
    ...
    // if(Get.find<ProviderBookingController>().categoryItemList.length == 2){
    //    int firstVisibleItem = visibleItems.first;
    //    int lastVisibleItem = visibleItems.last;
    //       if (notification.metrics.pixels <= 5) {
    //         widget._tabController.animateTo(firstVisibleItem);
    //       }
    //       else if (notification.metrics.pixels + notification.metrics.viewportDimension >=
    //           notification.metrics.maxScrollExtent) {
    //         widget._tabController.animateTo(lastVisibleItem);
    //       }else if (notification is ScrollUpdateNotification) {
    //         // Check if the scroll direction is going up
    //         if (notification.scrollDelta! < 0 && firstVisibleItem >= 0) {
    //           widget._tabController.animateTo(firstVisibleItem);
    //         } else {
    //           // Default case: Scroll direction is down, select the last tab
    //           widget._tabController.animateTo(lastVisibleItem);
    //         }
    //       }
    //
