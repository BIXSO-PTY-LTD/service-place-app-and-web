...
class _ConversationBubbleWidgetState extends State<ConversationBubbleWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    for (var element in imageList) {
      imagePathList.add(element.storedFileName??"");
    }
                        ...
                        child: SizedBox(width: 200,
                          child: GridView.builder(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              String imageUrl = '';
                              try{
                                imageUrl = '${Get.find<SplashController>().configModel.content!.imageBaseUrl}${AppConstants.conversationImagePath}'
                                    '${imageList[index].storedFileName ?? ''}';
                              }catch(e) {
                                if (kDebugMode) {
                                  print("");
                                }
                                ...
                                child: InkWell(
                                  onTap: (){
                                    Get.to(ImageDetailScreen(
                                      ...
                                      index: index,
                                      imagePath: AppConstants.conversationImagePath,
                                  ...
                                  onTap: ()async{
                                    if(ResponsiveHelper.isMobile(context)){
                                      ...
                                      if(status.isGranted){
                                        ...
                                        Get.find<ConversationController>().downloadFile(
                                          '${Get.find<SplashController>().configModel.content!.imageBaseUrl}'
                                              '${AppConstants.conversationImagePath}'
                                              '${fileList[index].storedFileName ?? ''}',
                                          directory!.path,
                                        );
                                      ...
                                      Get.find<ConversationController>().downloadFileForWeb(
                                          '${Get.find<SplashController>().configModel.content!.imageBaseUrl}'
                                              '/conversation/'
                                              '${widget.conversationData.conversationFile![index].storedFileName ?? ''}'
                                      );
                                    }
