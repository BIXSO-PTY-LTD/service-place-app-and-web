...
class ChannelItem extends StatelessWidget {
  ...
  final bool isAdmin;
  const ChannelItem({Key? key, required this.channelData, this.isAdmin = false,}) : super(key: key);
  @override
  Widget build(BuildContext context) {
    ...
    "${Get.find<SplashController>().configModel.content!.imageBaseUrl}$imagePath${Get.find<SplashController>().configModel.content?.favicon}"
        : conversationUser?.user?.userType == 'provider-admin' ? "${Get.find<SplashController>().configModel.content!.imageBaseUrl}$imagePath${conversationUser?.user?.provider?.logo}"
        : "${Get.find<SplashController>().configModel.content!.imageBaseUrl}$imagePath${conversationUser?.user?.profileImage}";
